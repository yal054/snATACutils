...
parser$add_argument("-f", "--frac", default = 0.1, help="fraction of cell have accessiable genes")
...
calFrac <- function(obj){
  gmat <- makeBinary(obj, "gmat")@gmat
  F <- Matrix::colSums(gmat) / nrow(gmat)
  return(F)
}
...
tic("calCnt")
CT <- Matrix::colSums(x.sp@gmat)
CNT1 <- Matrix::colSums(x.sp@gmat[which(x.sp@metaData$replicate == 1), ]) 
CNT2 <- Matrix::colSums(x.sp@gmat[which(x.sp@metaData$replicate == 2), ])
tic()
...
tic("Trinarize")
gmat <- makeBinary(x.sp.sub, "gmat")@gmat
k <- Matrix::colSums(gmat)
n <- nrow(gmat)
...
out <- data.frame(gene=colnames(x.sp@gmat), CT=CT, Cnt1=CNT1, Cnt2=CNT2, Frac=Frac, CPMnorm=CPM, RAnorm=RA, accessProb=tri_out$prob, accessPval=tri_out$pval, accessFDR=accessFDR, accessLabel=tri_out$access_label)
outfname = paste(outF, ".parseTmat.txt",sep="")
