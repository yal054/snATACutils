...
saveRDS(se.combined, file=paste(outF, "coembed.rds", sep="."))

#-------------------------
# plot & output results
pdf(paste(outF, "coembed.umap.pdf", sep="."), width = 18, height=6)
DimPlot(se.combined, reduction = "umap", group.by = c("set", "seurat_clusters"))
DimPlot(se.combined, group.by = c("seurat_clusters", "tech"))
DimPlot(se.combined, group.by = c("refCluster", "queryCluster")) + 
  theme(legend.position = "top") + guides(color = guide_legend(nrow = 4, byrow = TRUE,
    override.aes = list(size = 2.5)))
DimPlot(se.combined, group.by = c("refCluster", "queryCluster"), label = T) + 
  theme(legend.position = "top") + guides(color = guide_legend(nrow = 4, byrow = TRUE,
    override.aes = list(size = 2.5)))
dev.off()
